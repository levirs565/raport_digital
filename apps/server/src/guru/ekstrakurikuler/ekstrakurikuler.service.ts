...
export class GuruEkstrakurikulerService {
  ...
  async getAnggotaList(sessionUsername: string, id: string) {
    ...
    return result.Anggota_Ekstrakurikuler.map(({ nilai, keterangan, Siswa }) => ({
      ...Siswa,
      nilai,
      keterangan
    }))
  ...
  async updateAnggotaList(sessionUsername: string, id: string, siswaIdList: string[]) {
  ...
  async updateNilai(sessionUsername: string, id: string, nilai: NilaiEkstrakurikuler[]) {
    await this.ensureCanEdit(sessionUsername, id);

    await this.prismaClient.$transaction(
      nilai.map(({ id_siswa, nilai, keterangan }) => this.prismaClient.anggota_Ekstrakurikuler.update({
        where: {
