...
export class OperatorKelasRouter {
  ...
  router = this.trpc.router({
    ...
    getAll: this.trpc.operatorProcedure
      .input(periodeAjarIdSchema)
      .query(
        async ({ input }) => await this.service.getAll(input.periode_ajar_id)
      ),
      ...
      .mutation(
        async ({ input }) =>
          await this.service.add(
            ...
            input.kelas,
            input.kode_ruang_kelas,
            input.username_wali_kelas,
            input.username_koor_p5
          )
      ),
    ...
    getMataPelajaranList: this.trpc.operatorProcedure
      .input(idSchema)
      .query(
        async ({ input }) => await this.service.getMataPelajaranList(input.id)
      ),
    ...
    getMataPelajaran: this.trpc.operatorProcedure
      .input(baseMataPelajaranSchema)
      .query(
        async ({ input }) =>
          await this.service.getMataPelajaran(input.id, input.id_mata_pelajaran)
      ),
    deleteMataPelajaran: this.trpc.operatorProcedure
      .input(baseMataPelajaranSchema)
      .mutation(async ({ input }) => {
        await this.service.deleteMataPelajaran(
          input.id,
          input.id_mata_pelajaran
        );
        return true;
      }),
    updateMataPelajaran: this.trpc.operatorProcedure
      .input(
        baseMataPelajaranSchema.extend({
          username_guru: z.string(),
        })
      )
      .mutation(async ({ input }) => {
        await this.service.updateMataPelajaran(
          input.id,
          input.id_mata_pelajaran,
          input.username_guru
        );
